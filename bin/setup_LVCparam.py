...
def print_INFOS(INFOS):
  ...
  for item in INFOS:
    ...
    if isinstance(INFOS[item], list) or isinstance(INFOS[item], tuple):
      ...
      first = True
      for k, v in list(INFOS[item].items()):
        if i >= 10: break
        if first:
...
def reduce_displacement_dictionary_to_output_str(big_dictionary):
  ...
  displacement_list = []
  for normal_mode, displacement in list(big_dictionary.items()):
  ...
  for displacement in displacement_list:
    ...
    for normal_mode, disp in list(big_dictionary.items()):
...
def get_general():
  ...
  if not question('Do you want to make LVC parameters for all normal modes?', bool, True):
    not_using = question('Which of the normal modes do you NOT want to use? %s' % (reduce_big_list_to_short_str(list(INFOS['normal_modes'].keys()))), int, ranges = True)

    # delete selected normal modes & jump over non-existing ones
    for k in not_using:
      if not (k in list(INFOS['normal_modes'].keys())): print('Normal mode %i doesn\'t exist. Going on with the rest.' % (k))
      else: del INFOS['normal_modes'][k]

  # output to user
  print('\nWe will use the following normal modes: %s\n' % (reduce_big_list_to_short_str(list(INFOS['normal_modes'].keys()))))
  ...
  if INFOS['do_overlaps']:
    ...
    displacement_magnitudes = OrderedDict(sorted(list({ i: 0.05 for i in list(INFOS['normal_modes'].keys()) }.items()), key = lambda t: t[0]))
    ...
    if question('Do you want to use other displacements than the default of [0.05]?', bool, False):
      while True:
        ...
        indexes = question('For which normal modes do you want to use the displacement of %g? %s:' % (displacement, reduce_big_list_to_short_str(list(INFOS['normal_modes'].keys()))), int, ranges = True)
        
        # check if normal mode of input exists and add it to displacement_magnitudes dict
        for k in indexes:
          if k in list(INFOS['normal_modes'].keys()):
...
''')
    ...
    #saving input
    INFOS['one-sided_derivations'] = OrderedDict(sorted(list(one_sided_derivations.items()), key = lambda t: t[0]))

    # output to user
    print('\nOne-sided derivation will be used on: %s\n' % (reduce_big_list_to_short_str(list(one_sided_derivations.keys()))))
  ...
  if INFOS['do_overlaps']:
    INFOS['paths'].update({ k: 'DSPL_%03i_%s' % (int(k[:-1]), k[-1]) for k, v in list(INFOS['displacements'].items()) })
  # sort dict
  INFOS['paths'] = OrderedDict(sorted(list(INFOS['paths'].items()), key = lambda t: t[0]))
...
def read_V0(INFOS, content):
  ...
  for line in content:
    ...
    if header == headers['freq'] and line != headers['mwnmodes']:
      freqencies = { i + 1: freq for i, freq in zip(list(range(len(line.split()))), list(map(float, line.strip().split()))) }
    ...
    if line in list(headers.values()):
      header = line

  # save as ordered dict for nice output later on
  INFOS['normal_modes'] = OrderedDict(sorted(list({ i: normal_mode for i, normal_mode in list(normal_modes.items()) if freqencies[i] != 0 }.items()), key = lambda t: t[0]))
  INFOS['freqencies'] = OrderedDict(sorted(list({ i: freq for i, freq in list(freqencies.items()) if freq != 0 }.items()), key = lambda t: t[0]))
...
def calculate_displacements(INFOS):
  ...
  displacements = {}
  if INFOS['do_overlaps']:
    for k, normal_mode in list(fmw_normal_modes.items()):
  ...
  INFOS['displacements'] = OrderedDict(sorted(list(displacements.items()), key = lambda t: t[0]))
  return INFOS
...
def write_displacement_info(INFOS):
  ...
  if INFOS['do_overlaps']:
    for displacement_key, v in list(INFOS['displacements'].items()):
...
def setup_all(INFOS):
  ...
  if INFOS['do_overlaps']:
    ...
    number_of_displacements = len(list(INFOS['displacements'].keys()))

    # iterating through displacements
    for displacement_key, displacement_value in list(INFOS['displacements'].items()):
